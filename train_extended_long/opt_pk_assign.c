			...
			if(assign){
				...
				if(!opk->exn){
					if( need_refine){
						...
						poly[res]->nbeq = split_matrix(opk,poly[res]->C,C,ind_map_a,clv->size);
						//if(!is_pos){
						...
						//	poly[res]->C->nbrows++;
						//}
						poly[res]->nbline = split_matrix(opk,poly[res]->F,F,ind_map_a,clv->size); 
						...
						poly[num_comp]->nbeq = split_matrix(opk,poly[num_comp]->C,C,ind_map_b,clb_copy->size);
						//if(!is_pos){
						...
						//}
						poly[num_comp]->nbline = split_matrix(opk,poly[num_comp]->F,F,ind_map_b,clb_copy->size); 
