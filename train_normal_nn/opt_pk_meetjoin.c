...
opt_pk_array_t * opt_poly_join_gen(elina_manager_t *man, opt_pk_array_t *oa, opt_pk_array_t *ob, bool destructive){
	...
	**************************/
	...
	// 	if(r>=0.95){
	// 		threshold = (rand()%(max_block+1))+step_size;
	// 	}
	// 	else{
	// 		threshold = (max_block/step_size)*step_size + step_size;
	// 	}
	// }
	if(max_block <= 0){
		min_gen = 0;
	...
	for(k=0; k < num_comp; k++){
		if(pos_con_map[k]==2|| pos_con_map[k]==3){
			...
			while(c!=NULL){
				...
				if(!elina_interval_is_top(itv_a) && !elina_interval_is_top(itv_a)){
					if(elina_scalar_infty(itv_a->sup) || elina_scalar_infty(itv_a->inf)){
						if(elina_scalar_infty(itv_b->sup) || elina_scalar_infty(itv_b->inf)){
							num_half_top++;
						}
	...
	PyObject* pName = PyUnicode_DecodeFSDefault("call_nn_dqn");
  ...
  // free_array_comp_list(acl_finest);
	...
	if(max_block_gt_0){
		int cpuCylces = (int) log10((double) finished - started);

		reward = 3*num_points + 2*num_bounded + num_half_top - cpuCylces;
		...
		PyObject* countn = PyObject_CallObject(pFunc, pArgs);
		int count = (int) PyLong_AsLong(countn);
		// if(count == 149){
