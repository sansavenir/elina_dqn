...
void opt_pk_internal_init(opt_pk_internal_t* opk, size_t maxdims)
{
 ...
 /* unsigned long var = (unsigned long)time(NULL);
  const int n = snprintf(NULL, 0, "%lu", var);
  opk->filename = (char*)malloc((n+6)*sizeof(char));
  int c = snprintf(opk->filename, n+1, "%lu", var);
  strcat(opk->filename,"data_rl.txt");*/
  FILE *fp = fopen("weights.txt","r");
  opk->weights = (double **)malloc(36*sizeof(double*));
  for(int i=0; i < 36; i++){
	opk->weights[i] = (double*)calloc(80,sizeof(double));
  }
  for(int i=0; i < 36; i++){
	for(int j=0; j < 80; j++){
		
		fscanf(fp,"%lf",&opk->weights[i][j]);
...
elina_manager_t* opt_pk_manager_alloc(bool strict)
{
  ...
  man = elina_manager_alloc(strict ? "optpoly, strict mode" : "optpoly, loose mode","1.0 with Long Long Int",
  ...
  funptr[ELINA_FUNID_TO_BOX] = &opt_pk_to_box_wrapper;
