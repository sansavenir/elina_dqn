...
opt_pk_array_t * opt_poly_join_gen(elina_manager_t *man, opt_pk_array_t *oa, opt_pk_array_t *ob, bool destructive){
	...
	//fflush(stdout);
	//FILE *fp = fopen("/tmp/seahorn.txt","a");
       // fprintf(fp,"%d\n",oa->maxcols-2);
       // fflush(fp);
	//fclose(fp);
	//srand ( time(NULL) );
	...
	**************************/
	...
	for(k=0; k < num_comp; k++){
		...
		if(!nbmapFa[k] || !nbmapFb[k]){
			...
			while(c!=NULL){
				...
				if(!elina_interval_is_top(itv_a) && !elina_interval_is_top(itv_b)){
					if(elina_scalar_infty(itv_a->sup) || elina_scalar_infty(itv_a->inf)){
						if(elina_scalar_infty(itv_b->sup) || elina_scalar_infty(itv_b->inf)){
							num_half_top++;
						}
					}
					else{
	...
	if(max_block>0){
		...
		int state[9];
  		state[0] = num_blocks > 9 ? 9 : num_blocks - 1;
		unsigned short int tmp =  min_block/10;
		state[1] = (tmp > 9) ? 9 : tmp; 
		tmp = max_block /10;
		state[2] = (tmp > 9) ? 9 : tmp;
		tmp = avg_blocks/10;
		...
		for(int i=0; i < 36; i++){
			double sum_action_value = 0;
			for(int j =0; j < 7; j++){
				sum_action_value = sum_action_value + opk->weights[i][j*10+state[j]];
			}
			sum_action_value = sum_action_value + opk->weights[i][70+state[7]];
			sum_action_value = sum_action_value + opk->weights[i][75+state[8]];
			if(sum_action_value > max_action_value){
				max_action_value = sum_action_value;
				action = i;
			}
		}
		// for(int i = 0; i < 9; i++){
		...
		PySys_SetPath(Path);
			PyObject* pName = PyUnicode_DecodeFSDefault("count_joins");
		PyObject* pModule = PyImport_Import(pName);

		PyObject* pFunc = PyObject_GetAttrString(pModule, "count");
		...
		ss = tmp/3;
		ms = tmp%3;
       // printf("first %d %d %d %d %d %d %d %g %lld %lld %g\n",threshold,ss==BINARY_WEIGHT_DELETE,ms==MERGE_SMALL,maxcols,num_blocks,min_block,max_block,avg_blocks,min_gen, max_gen,avg_gen);
       // fflush(stdout);
	}
	else{
		min_gen = 0;
	...
	for(k=0; k < num_comp; k++){
		...
		//fprintf(fp,"%u %u %u %g ",num_blocks,min_block,max_block,avg_blocks);
		//fprintf(fp,"%zu %zu %g ",min_gen,max_gen,avg_gen);
		//fprintf(fp,"%u %u %zu %d %d ",num_half_top,num_points,threshold,ss,ms);
		//fflush(fp);
				...
				while(fcl!=NULL){
					...
					if(fcl->size<=threshold){
						...
						if(newCl->size>threshold){
							...
							//printf("maxflow output %d %d\n",C1->nbrows,newC->nbrows);
							//opt_matrix_fprint(stdout,newC);
							//fflush(stdout);
  ...
  // free_array_comp_list(acl_finest);
	...
	num_half_top=0;
	num_points = 0;
	unsigned short int num_bounded = 0;
	...
	reward = 3*num_points + 2*num_bounded + num_half_top;
	...
	if(max_block > 0 ){
		...
		PyObject* pFunc = PyObject_GetAttrString(pModule, "saveReward");

		PyObject* pArgs = PyTuple_New(2);
		PyObject* pValue = PyLong_FromLong(reward);
		PyTuple_SetItem(pArgs, 0, pValue);
		double cycles = (double)(finished - started);
		pValue = PyLong_FromLong(cycles);
